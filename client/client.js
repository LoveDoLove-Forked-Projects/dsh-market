window.__ModuleLoader__.load({ id: "dshmarket", factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			setBuildEnv: "编译环境变量",
			setBuildEnvHint: "会进入市场启动的每一个子进程（安装、编译、git 等），不只是编译器。每行一个 KEY=value，例如 CC=/usr/bin/gcc-11；PATH 和 CI 由市场接管。留空并保存即恢复 profile 配置。",
			...
			capLlm: "调用模型",
			capSubagent: "起子代理",
			capDynamicCode: "动态执行代码",
			capHostRuntime: "宿主运行时",
			capInstallScript: "安装脚本",
			capabilityNone: "未检出",
			capabilityUnchecked: "未扫描",
			capabilityMore: "+{0}",
			capabilityRedLine: "值得看一眼：{0}",
			capabilityNote: "静态扫描「安装时会下载的那个包」得出的结果，只证有不证无：跳过 node_modules，运行时拼接的地址、动态 import、混淆代码都扫不到。「未检出」不等于安全，也不是我们的评价，请自行判断。",
			capabilityScannedAt: "扫描于 {0}",
			capabilityShowAll: "展开全部",
			capRedCredentialsNetwork: "同时读凭据并联网",
			capRedPlaintextHttp: "用明文 http:// 连接 {0}",
			capabilityHideAll: "收起",
			favoriteAdd: "加入收藏",
			favoriteRemove: "取消收藏",
		...
		const en = {
			...
			setBuildEnvHint: "Reaches every process the market starts — installs, builds, git — not only the compiler. One KEY=value per line, e.g. CC=/usr/bin/gcc-11. PATH and CI are managed by the market. Saving an empty list restores the profile config.",
			...
			capabilityTitle: "What it touches",
			capShell: "runs commands",
			capFsRead: "reads files",
			capFsWrite: "writes files",
			capNetwork: "network",
			capCredentials: "reads credentials",
			capEnv: "reads env",
			capLlm: "calls a model",
			capSubagent: "spawns subagents",
			capDynamicCode: "dynamic code",
			capHostRuntime: "host runtime",
			capInstallScript: "install script",
			capabilityNone: "not detected",
			capabilityUnchecked: "not checked",
			capabilityMore: "+{0}",
			capabilityRedLine: "Worth a look: {0}",
			capabilityNote: "A static scan of the package this install would download. It reports only what it found: node_modules is skipped, and runtime-assembled URLs, dynamic imports and obfuscated code are invisible to it. “Nothing detected” is not “safe” — it is not a judgement, and the call is yours.",
			capabilityScannedAt: "Scanned {0}",
			capabilityShowAll: "Show all",
			capRedCredentialsNetwork: "reads credentials and has network access",
			capRedPlaintextHttp: "uses plaintext http:// to {0}",
			capabilityHideAll: "Show less",
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								p.capabilities === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								}) : p.capabilities.map((name) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
